...
`텍스트 컨텍스트 메뉴` → `왼쪽에서 오른쪽으로`는 텍스트 상자의 기본값입니다.

텍스트 상자가 동적(Dynamic) 모드, 고정(Fixed) 모드인 경우 모두 같은 방식으로 입력됩니다. 아래 동영상에서 확인해 볼 수 있습니다.
...
`김프 텍스트 편집기` → `왼쪽에서 오른쪽으로`을 선택해서 입력할 수 있습니다.
