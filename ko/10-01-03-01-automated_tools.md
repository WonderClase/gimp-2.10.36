...
이 대화상자의 아래쪽에 있는 `입력 레벨 자동화`을 클릭합니다. 결과에 대한 미리보기가 가능하며, 효과를 적용하려면 확인 버튼을 누릅니다.

만약 취소 버튼을 누르면 이미지는 이전 상태로 되돌아갑니다.
...
만약 이미지에서 완전하게 하얀색이거나 검정색인 부분이 있다면, 반자동 조정모드로 레벨 도구를 사용할 수 있습니다. 이는 밝기나 색상 조절에 좋은 방법입니다.

먼저 앞서 말한 레벨 도구를 불러옵니다.

그런 후에 레벨 대화상자의 아래쪽에 있는 스포이드 모양의 세 개의 아이콘을 찾습니다. 
...
원하는 결과가 나왔다면 확인 버튼을 누르고, 그렇지 않다면 취소 버튼을 누릅니다.
